...
class UserFiles extends Command {
	...
	public function execute(InputInterface $input, OutputInterface $output): int {
		...
		$output->writeln("");
		$count = (int)$input->getOption("large-files");
		$output->writeln("  large files and folders: ");
		$this->fileUtils->outputLargeFilesTree($output, $this->rootFolder->getUserFolder($user->getUID()), "    ", array_fill(0, $count, 0));
