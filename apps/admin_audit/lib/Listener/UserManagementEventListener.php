...
class UserManagementEventListener extends Action implements IEventListener {
	...
	private function userCreated(UserCreatedEvent $event) {
		$this->log(
			'User created: "%s"',
			[
				'uid' => $event->getUid()
	...
	private function userDeleted(UserDeletedEvent $event) {
		$this->log(
			'User deleted: "%s"',
			[
	...
	private function userChanged(UserChangedEvent $event) {
	...
	private function passwordUpdated(PasswordUpdatedEvent $event) {
