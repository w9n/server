...
class AuthEventListener extends Action implements IEventListener {
	...
	private function beforeUserLoggedIn(BeforeUserLoggedInEvent $event) {
		$this->log(
			'Login attempt: "%s"',
			[
	...
	private function userLoggedIn(UserLoggedInWithCookieEvent|UserLoggedInEvent $event) {
		$this->log(
			'Login successful: "%s"',
			[
	...
	private function beforeUserLogout(BeforeUserLoggedOutEvent $event) {
		$this->log(
			'Logout occurred',
			[],
			[]
